package com.ksa.system.initialize.model;


public class KeHu {

    private String daiMa;
    private String zhongWMC;
    private String zhongWJC;
    private String zhongWDZ;
    private String tiDan;
    private String beiZhu;
    private boolean changYong;
    private String fuKuanZQ;
    private String leiXing;
    
    public String getDaiMa() {
        return daiMa;
    }
    
    public void setDaiMa( String daiMa ) {
        this.daiMa = daiMa;
    }
    
    public String getZhongWMC() {
        return zhongWMC;
    }
    
    public void setZhongWMC( String zhongWMC ) {
        this.zhongWMC = zhongWMC;
    }
    
    public String getZhongWDZ() {
        return zhongWDZ;
    }
    
    public void setZhongWDZ( String zhongWDZ ) {
        this.zhongWDZ = zhongWDZ;
    }
    
    public String getTiDan() {
        return tiDan;
    }
    
    public void setTiDan( String tiDan ) {
        this.tiDan = tiDan;
    }
    
    public String getBeiZhu() {
        return beiZhu;
    }
    
    public void setBeiZhu( String beiZhu ) {
        this.beiZhu = beiZhu;
    }
    
    public boolean isChangYong() {
        return changYong;
    }
    
    public void setChangYong( boolean changYong ) {
        this.changYong = changYong;
    }
    
    public String getFuKuanZQ() {
        return fuKuanZQ;
    }
    
    public void setFuKuanZQ( String fuKuanZQ ) {
        this.fuKuanZQ = fuKuanZQ;
    }
    
    public String getLeiXing() {
        return leiXing;
    }
    
    public void setLeiXing( String leiXing ) {
        this.leiXing = leiXing;
    }

    public String getZhongWJC() {
        return zhongWJC;
    }

    public void setZhongWJC( String zhongWJC ) {
        this.zhongWJC = zhongWJC;
    }
    
}
